...
from torch_geometric.nn import EdgeConv, fps
import time
import tqdm
...
def edge_mlp(in_dim, out_dim):
    """MLP for an EdgeConv layer: maps the per-edge feature [x_i ‖ x_j - x_i]
    (size 2*in_dim) to out_dim."""
...
class EdgeConvBranch(nn.Module):
    """3-layer EdgeConv (DGCNN-style) feature extractor for a single resolution.
    Each layer learns an edge function over [x_i, x_j - x_i] and max-aggregates."""

    def __init__(self, in_dim=6, hid=HID_DIM):
        super().__init__()
        self.conv1 = EdgeConv(edge_mlp(in_dim, hid), aggr="max")
        self.conv2 = EdgeConv(edge_mlp(hid, hid), aggr="max")
        self.conv3 = EdgeConv(edge_mlp(hid, hid), aggr="max")
...
class MultiScaleNodeEncoder(nn.Module):
    ...
    Each scale has its own EdgeConv branch; the coarse features are gathered
    back onto the full nodes (via nearest-sampled-node maps) and concatenated,
    then an MLP head fuses them into the final L2-normalised embedding."""

    def __init__(self, in_dim=6, hid=HID_DIM, out=EMB_DIM, n_coarse=2):
        super().__init__()
        self.full_branch = EdgeConvBranch(in_dim, hid)
        self.coarse_branches = nn.ModuleList(
            [EdgeConvBranch(in_dim, hid) for _ in range(n_coarse)]
        )
...
def random_rotation_matrix(device):
    """A uniformly random proper 3D rotation matrix (det = +1), via QR."""
    q, r = torch.linalg.qr(torch.randn(3, 3, device=device))
    q = q * torch.sign(torch.diagonal(r))     # Haar-uniform sign fix
    if torch.det(q) < 0:                       # ensure a proper rotation
        q[:, 0] = -q[:, 0]
    return q


def rotate_features(x, rot):
    """Rotate the position and normal blocks of a feature matrix by `rot`."""
    pos = x[:, :3] @ rot.T
    nrm = x[:, 3:] @ rot.T
    return torch.cat([pos, nrm], dim=1)


def rotate_ms(ms, rot):
    """Apply the same 3D rotation to features at every scale (positions and
    normals); graph structure and maps are unchanged. Used to enforce
    rotation invariance of the encoder."""
    out = dict(ms)
    out["x"] = rotate_features(ms["x"], rot)
    out["coarse"] = [{"x": rotate_features(c["x"], rot), "ei": c["ei"]}
                     for c in ms["coarse"]]
    return out


def normalize_mesh_features(mesh):
    """Per-mesh feature normalization, returning (pos, normals) tensors:
      - positions are centered at the centroid and scaled to a unit bounding
        sphere, so geometry is translation/scale invariant and comparable
        across meshes (the raw cached positions have per-mesh scale);
      - normals are made unit-length, with degenerate (zero-length) normals
        replaced by a default up vector.
    """
    pos = mesh.pos.float()
    center = pos.mean(0, keepdim=True)
    pos = pos - center
    scale = pos.norm(dim=1).max().clamp(min=1e-6)
    pos = pos / scale                                    # unit bounding sphere

    nrm = mesh.normals.float()
    bad = nrm.norm(dim=1) < 1e-6                          # degenerate normals
    if bad.any():
        nrm = nrm.clone()
        nrm[bad] = torch.tensor([0.0, 0.0, 1.0], dtype=nrm.dtype)
    nrm = F.normalize(nrm, dim=1)                         # ensure unit length
    return pos, nrm
...
def build_multiscale_graph(mesh, samplings, device):
    ...
    Positions are normalized per mesh and normals cleaned (see
    normalize_mesh_features) before being used as input features.
    """
    pos, nrm = normalize_mesh_features(mesh)
    x_full = torch.cat([pos, nrm], dim=1).to(device)
    ei_full = mesh.edge_index.long().to(device)
    nf = x_full.size(0)
    adj = torch.sparse_coo_tensor(
        ei_full, torch.ones(ei_full.size(1), device=device), (nf, nf)
    ).coalesce()
    pos_full = pos.cpu().numpy()

    coarse, maps = [], []
    for samp in samplings:
        idx = samp.indices.long()
        # Same per-mesh transform applied to the sampled subset (keeps scales aligned).
        xc = torch.cat([pos[idx], nrm[idx]], dim=1).to(device)
        eic = samp.edges.long().to(device)
        coarse.append({"x": xc, "ei": eic})
        # Nearest sampled node for every full node (Euclidean in position).
        nn_idx = KDTree(pos[idx].cpu().numpy()).query(pos_full, k=1)[1][:, 0]
...
def visualize_embeddings(encoder, device, n_meshes=5, off_screen=False,
    ...
    # Pass 2: lay out as a single row up to 5 meshes, otherwise a near-square
    # grid that grows with the count.
    if n_meshes <= 5:
        rows, cols = 1, n_meshes
    else:
        cols = int(np.ceil(np.sqrt(n_meshes)))
        rows = int(np.ceil(n_meshes / cols))

    plotter = pv.Plotter(shape=(rows, cols), window_size=[360 * cols, 360 * rows],
...
def train(cfg):
    ...
    for epoch in range(cfg.epochs):
        ...
        for _ in tqdm.trange(cfg.iters, desc=f"epoch {epoch}", leave=False):
            ...
            emb_rot = encoder(rotate_ms(g, random_rotation_matrix(device)))
            ...
            p3 = emb_rot[anchors]       # positive: same node, mesh rotated (invariance)
            ...
            # (3 positives × 2 negatives = 6 triplets per anchor).
            positives = [p1, p2, p3]
            negatives = [n1, n2]
            a_rep = torch.cat([a] * (len(positives) * len(negatives)))
            pos = torch.cat([p for _ in negatives for p in positives])
            neg = torch.cat([n for n in negatives for _ in positives])
...
if __name__ == "__main__":
    ...
    ap.add_argument("--n-meshes", type=int, default=12, dest="n_meshes",
                    help="how many meshes to visualize (>5 uses a grid layout)")
    ...
    visualize_embeddings(encoder, device, n_meshes=cfg.n_meshes,
