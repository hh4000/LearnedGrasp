...
from graspcnn.losses import GaussianNLLLoss
from graspcnn.models import MANOGraspCNNv1, MANOGraspCNNv2, MANOGraspCNNv3
from graspcnn.data import GraspImagePosesDataset, GraspImagePosesDatasetV2
...
class _RegressionTrainer(Trainer):
    """Image→MANO-pose regression trainer with PCA-variance-weighted loss."""

    def __init__(self, *args, pca_variances: torch.Tensor, **kwargs):
        super().__init__(*args, **kwargs)
        self.pca_variances = pca_variances[:self.n_outputs]

    def forward_batch(self, item):
        inputs, labels = item
        inputs = inputs.to(self.device, non_blocking=True).float()
        labels = labels.to(self.device, non_blocking=True)[:, :self.n_outputs]
        return self.model(inputs), labels

    def reduce_loss(self, loss):
        if isinstance(loss, torch.Tensor) and loss.numel() > 1:
            loss = (loss / self.pca_variances).mean()
        return loss
...
def training_thread(cfg: dict, result_q: queue.Queue, stop_evt: threading.Event):
    try:
        ...
        trainer = _RegressionTrainer(model, criterion, optimizer, scheduler,
